...
| La caja la cierra quien la trabaja | `POST /modulos_caja/:id/jornada` con `resultado: TERMINADA` (`declararJornadaDeCaja`): cierra la caja atribuida a la fecha de su último registro y anota la jornada con su conteo. `CONTINUA` se retiró y responde 400 | `CierreDeJornada.tsx`: "Terminé esta caja" mientras la caja está EN PROCESO |
| Cambio de estado a mano | `PATCH /modulos_caja/:id/cambiarEstado`: TECNICA solo puede terminar sus cajas (ponerlas EN PROCESO responde 403 con el código `REAPERTURA_REQUIERE_LIDER`), LIDER en las de su sede, ADMIN en todas. Al finalizar, la fecha y la persona salen del último registro de la caja. Si un líder o administrador la pone EN PROCESO se escriben `reabierta_por` y `reabierta_el`, todo cierre los borra, y las solicitudes de reapertura pendientes quedan aprobadas y avisadas | `CajasPage.tsx` y `ActasPage.tsx`: "Reabrir caja" para el líder, "Dar por terminada" para la técnica |
| Una caja terminada no se le reabre a la técnica digitando en ella | `createFuid` en `fuiddatosreal.controller.ts`: si la caja está FINALIZADO y quien guarda no es LIDER ni ADMIN, 403 con `REAPERTURA_REQUIERE_LIDER` | `DatosPage.tsx` muestra `CajaTerminada.tsx` en lugar del formulario |
| La reapertura se pide al líder y se aprueba desde el software | `POST /modulos_caja/:id/solicitar-reapertura` (TECNICA con la caja asignada y la caja terminada; una pendiente por persona y caja, índice único parcial); `POST /solicitudes_reapertura/:id/aprobar` y `/rechazar` (LIDER de la sede o ADMIN). Aprobar reabre la caja firmada por el líder y avisa a quien pidió (`reaperturaCaja.service.ts`) | `NotificacionesMenu.tsx`: el líder aprueba o rechaza desde la campana; `CajaTerminada.tsx`: la técnica pide |
| Los avisos llegan al instante y no se pierden | `notificaciones.service.ts`: cada aviso se guarda en `notificacion` y se escribe en la conexión abierta (`GET /notificaciones/stream`, eventos del servidor) de quien lo recibe | `useNotificaciones.ts`: `EventSource` mientras la pestaña está en primer plano; sondeo si no hay conexión |
