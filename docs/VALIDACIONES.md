...
| Reabrir una caja no toca el consecutivo de UPD | `cambiarEstadoCaja` a EN PROCESO deja intactos `upd_inicio` y `ultimo_upd`: `GET /modulos_caja/next-upd/:caja` sigue devolviendo el siguiente del que ya venía. Solo responde `requiere_inicio` la primera vez que esa persona entra a esa caja | `UpdInicioDialog` en `DatosPage.tsx`, solo la primera vez y con la caja abierta |
| A quien ya digitó en una caja nunca se le pide el arranque | `getNextUpdByCaja`: si no hay `upd_inicio` ni `ultimo_upd`, busca en `fuiddatosreal` el mayor UPD de esa persona en esa caja y continúa desde ahí. `asignacion_caja_tecnica` es un espejo y se puede perder —lo perdieron las cajas reabiertas antes del 24 de septiembre de 2026—; los registros, no. El consecutivo de otra persona en la misma caja no cuenta: se compara por `elaborado_por` | `UpdInicioDialog` no aparece |
