...
export const AJUSTES: AjusteDeEsquema[] = [
  ...
  {
    /*
     * Reapertura de cajas por solicitud. La técnica no reabre una caja
     * terminada por su cuenta: lo pide, el líder lo autoriza y ambos se
     * enteran al instante. Cada petición queda aquí con quién, sobre qué caja,
     * en qué sede y cómo se resolvió (`reaperturaCaja.service.ts`).
     */
    nombre: 'tabla solicitud_reapertura (la técnica pide reabrir una caja terminada)',
    sql: `CREATE TABLE IF NOT EXISTS solicitud_reapertura (
            id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  ...
  {
    // Una sola pendiente por persona y caja: volver a pulsar devuelve la que ya está.
    nombre: 'índice único solicitud_reapertura pendiente (una por persona y caja)',
    sql: `CREATE UNIQUE INDEX IF NOT EXISTS uq_solicitud_reapertura_pendiente
            ON solicitud_reapertura (caja_id, solicitante_id) WHERE estado = 'PENDIENTE'`,
  },
  {
    nombre: 'índice solicitud_reapertura(caja_id) (las solicitudes de una caja)',
    sql: 'CREATE INDEX IF NOT EXISTS idx_solicitud_reapertura_caja ON solicitud_reapertura (caja_id)',
  },
  {
    nombre: 'índice solicitud_reapertura(solicitante_id) (las solicitudes de una persona)',
    sql: 'CREATE INDEX IF NOT EXISTS idx_solicitud_reapertura_solicitante ON solicitud_reapertura (solicitante_id)',
  },
  {
    /*
     * Los avisos de cada persona: la solicitud que le llega al líder, y la
     * respuesta que le llega a la técnica. La entrega inmediata va por una
     * conexión abierta (`notificaciones.service.ts`); la tabla es lo que hace
     * que nada se pierda si no estaba conectada en ese momento.
     */
    nombre: 'tabla notificacion (avisos dentro del software)',
    sql: `CREATE TABLE IF NOT EXISTS notificacion (
            id integer GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  ...
  {
    nombre: 'índice notificacion(usuario_id, leida_en) (la campana de cada persona)',
    sql: 'CREATE INDEX IF NOT EXISTS idx_notificacion_usuario ON notificacion (usuario_id, leida_en)',
  },
  {
    /*
